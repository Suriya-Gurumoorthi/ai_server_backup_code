    ...
    if isinstance(message, bytes):
        if connection["waiting_for_audio"]:
            ...
            if request_type == "transcribe":
                ...
                # -- Background Task: Transcribe for logging (non-blocking) --
                async def log_transcription(audio, conn_id):
                    try:
                        # Create a simple transcription prompt
                        transcription_turns = [
                            {"role": "system", "content": "You are a speech-to-text transcription engine. Transcribe the audio exactly as spoken. Do not add commentary or corrections."},
                            {"role": "user", "content": "Please transcribe this audio: <|audio|>"}
                        ]
                        user_transcription = await process_audio_message(audio, transcription_turns)
                        logger.info(f"[BACKGROUND] User transcription for logging: {user_transcription[:50]}...")
                    except Exception as e:
                        logger.error(f"[BACKGROUND] Error transcribing audio for logging: {e}")
                
                # Fire off transcription task in background (no await - continues immediately)
                asyncio.create_task(log_transcription(message, connection_id))
                ...
                logger.info(f"Generating TTS audio for {client_address}")
            ...
            # -- Critical Path: Direct audio-to-AI response (fastest possible) --
            logger.info(f"Processing audio for AI response for {client_address}")
            response_text = await process_audio_message(message, connection["conversation_turns"])
            logger.info(f"AI Response: {response_text[:100]}...")
