...
class VoiceActivityDetector:
    ...
    def __init__(self, 
        ...
        self.sustained_speech_start_time = 0.0
        self.sustained_speech_duration = 0.0
        self.min_sustained_speech_duration = 1.0  # 1 second minimum for barge-in
        self.is_sustained_speech = False
        self.last_speech_detection_time = 0.0
    ...
    def is_speech_for_barge_in(self, audio_data: bytes) -> bool:
        """
        Sustained speech detection for barge-in that requires 1+ seconds of continuous speech
        to prevent false triggers from brief noise like paper rustling or chain movement
        
        Args:
            audio_data: Raw 16-bit PCM audio data
            
        Returns:
            True if sustained speech is detected (1+ seconds), False otherwise
        """
        if not audio_data or len(audio_data) < 2:
            return False
        ...
        # Enhanced speech detection criteria with higher thresholds for barge-in
        # Use higher energy threshold to reduce false positives from background noise
        barge_in_energy_threshold = self.energy_threshold * 1.5  # 50% higher threshold
        has_energy = rms_energy > barge_in_energy_threshold or peak_amplitude > barge_in_energy_threshold
        has_activity = zcr > 0.02  # Higher ZCR requirement (2% instead of 1%)
        is_not_noise = spectral_flatness < (self.spectral_flatness_threshold * 0.8)  # Stricter noise filtering
        ...
        if basic_speech:
            if self.sustained_speech_start_time == 0.0:
                # Start of potential sustained speech
                self.sustained_speech_start_time = current_time
                self.last_speech_detection_time = current_time
                logger.debug(f"VAD: Potential sustained speech started at {current_time:.2f}")
            else:
                # Continue sustained speech
                self.last_speech_detection_time = current_time
                self.sustained_speech_duration = current_time - self.sustained_speech_start_time
                
                # Check if we've reached the minimum sustained speech duration
                if self.sustained_speech_duration >= self.min_sustained_speech_duration:
                    if not self.is_sustained_speech:
                        self.is_sustained_speech = True
                        logger.info(f"VAD: Sustained speech detected! Duration: {self.sustained_speech_duration:.2f}s")
        else:
            # No speech detected - reset sustained speech tracking
            if self.sustained_speech_start_time > 0.0:
                # Check if we had a gap in speech (more than 0.5 seconds)
                time_since_last_speech = current_time - self.last_speech_detection_time
                if time_since_last_speech > 0.5:  # 500ms gap resets sustained speech
                    logger.debug(f"VAD: Speech gap detected ({time_since_last_speech:.2f}s) - resetting sustained speech")
                    self.sustained_speech_start_time = 0.0
                    self.sustained_speech_duration = 0.0
                    self.is_sustained_speech = False
        ...
        if VAD_DEBUG_LOGGING:
            logger.debug(f"VAD Barge-in Analysis: rms={rms_energy:.1f}, peak={peak_amplitude}, "
                        f"zcr={zcr:.3f}, flatness={spectral_flatness:.3f}, "
                        f"sustained_duration={self.sustained_speech_duration:.2f}s, "
                        f"basic={basic_speech}, sustained={self.is_sustained_speech}")
            
            # Additional debug info for sustained speech tracking
            if self.sustained_speech_start_time > 0.0:
                logger.debug(f"VAD Sustained Speech: start_time={self.sustained_speech_start_time:.2f}, "
                           f"duration={self.sustained_speech_duration:.2f}s, "
                           f"min_required={self.min_sustained_speech_duration}s")
        
        return self.is_sustained_speech
    
    def reset_sustained_speech(self):
        """
        Reset sustained speech detection state (call after barge-in is processed)
        """
        self.sustained_speech_start_time = 0.0
        self.sustained_speech_duration = 0.0
        self.is_sustained_speech = False
        self.last_speech_detection_time = 0.0
        logger.debug("VAD: Sustained speech state reset")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            # Sustained speech detection stats
            'sustained_speech_start_time': self.sustained_speech_start_time,
            'sustained_speech_duration': self.sustained_speech_duration,
            'is_sustained_speech': self.is_sustained_speech,
            'min_sustained_speech_duration': self.min_sustained_speech_duration
        }
