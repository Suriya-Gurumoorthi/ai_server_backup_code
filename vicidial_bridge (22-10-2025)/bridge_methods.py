...
class BridgeMethods:
            ...
            while call_id in self.bridge.active_calls:
                ...
                if response:
                    if response["type"] == "text":
                        # Process text response from AI (legacy support)
                        ...
                        # Convert text to speech and send back to Vicial with current turn ID
                        current_turn_id = call_info.get('current_turn_id', 0)
                        await self._process_ai_text_response(call_id, ai_text, current_turn_id)
                        
                    elif response["type"] == "audio":
                        # Process audio response from AI (direct audio from Ultravox - PRIMARY PATH)
                        audio_data = response["content"]
                        call_info['ai_responses_received'] += 1
                        logger.info(f"[{call_id}] 🎵 Received AI audio response #{call_info['ai_responses_received']}: {len(audio_data)} bytes")
                        
                        # Process direct audio response with current turn ID
                        current_turn_id = call_info.get('current_turn_id', 0)
                        await self._process_ai_audio_response(call_id, audio_data, current_turn_id)
                        ...
                        if binary_data.startswith(b'RIFF') and b'WAVE' in binary_data[:12]:
                            ...
                            await self._process_ai_audio_response(call_id, binary_data)
    ...
    async def _process_ai_text_response(self, call_id: str, ai_text: str, turn_id: int):
        """Convert AI text response to speech using Piper TTS and send back to Vicial with turn control"""
        # IMMEDIATE EXIT: Check if call is still active before processing
        if call_id not in self.bridge.active_calls:
            logger.debug(f"[{call_id}] Skipping AI text processing - call no longer active")
            return
            
        call_info = self.bridge.active_calls.get(call_id)
        if not call_info or call_info.get('call_ended', False):
            logger.debug(f"[{call_id}] Skipping AI text processing - call ended")
            return
        
        try:
            local_model_client = call_info['local_model_client']
            call_info['tts_requests_sent'] += 1
            
            logger.info(f"[{call_id}] 🎤 Generating TTS audio #{call_info['tts_requests_sent']} for: {ai_text[:50]}...")
            
            # Generate speech using Piper TTS
            tts_wav_data = await local_model_client.piper_tts.generate_speech(ai_text)
            
            if tts_wav_data:
                call_info['tts_responses_received'] += 1
                logger.info(f"[{call_id}] 🎵 Generated TTS audio #{call_info['tts_responses_received']}: {len(tts_wav_data)} bytes")
                
                # Convert TTS audio to 8kHz and send to Vicial with turn control
                await self._send_tts_audio_to_vicial(call_id, tts_wav_data, turn_id)
            else:
                logger.error(f"[{call_id}] Failed to generate TTS audio for: {ai_text[:50]}...")
                
        except Exception as e:
            logger.error(f"[{call_id}] Error processing AI text response: {e}")
    ...
    async def _process_ai_audio_response(self, call_id: str, audio_wav_data: bytes, turn_id: int):
        ...
        try:
            logger.info(f"[{call_id}] 🎵 Processing direct audio response from Ultravox: {len(audio_wav_data)} bytes")
            
            # BARGE-IN SUPPORT: Check if this turn is still current before processing
    ...
    async def _send_tts_audio_to_vicial(self, call_id: str, tts_wav_data: bytes, turn_id: int = None):
        """Convert TTS audio to 8kHz and send to Vicial via AudioSocket with turn-based control"""
        # IMMEDIATE EXIT: Check if call is still active before processing
        if call_id not in self.bridge.active_calls:
            logger.debug(f"[{call_id}] Skipping TTS audio send - call no longer active")
            return
            
        call_info = self.bridge.active_calls.get(call_id)
        if not call_info or call_info.get('call_ended', False):
            logger.debug(f"[{call_id}] Skipping TTS audio send - call ended")
            return
        
        # BARGE-IN SUPPORT: Use current turn ID if not provided
        if turn_id is None:
            turn_id = call_info.get('current_turn_id', 0)
        
        logger.info(f"[{call_id}] 🎤 Starting TTS playback for turn {turn_id}")
        
        try:
            local_model_client = call_info['local_model_client']
            
            # Convert WAV to 16kHz PCM using Piper TTS converter
            pcm_16k = local_model_client.piper_tts.convert_wav_to_audiosocket_format(tts_wav_data, target_sample_rate=16000)
            
            if pcm_16k:
                logger.info(f"[{call_id}] 🎵 Converted TTS to 16kHz PCM: {len(pcm_16k)} bytes")
                
                # ENHANCED DEBUG: Analyze 16kHz PCM audio
                samples_16k = len(pcm_16k) // 2
                duration_16k_ms = (samples_16k / 16000) * 1000
                max_amp_16k = max(abs(s) for s in struct.unpack(f'<{samples_16k}h', pcm_16k)) if samples_16k > 0 else 0
                logger.info(f"[{call_id}] 📊 16kHz PCM: {samples_16k} samples, {duration_16k_ms:.1f}ms, max_amp={max_amp_16k}")
                
                # Downsample from 16kHz to 8kHz for Vicial
                pcm_8k = self.bridge.audio_processor.downsample_16k_to_8k(pcm_16k)
                
                if pcm_8k:
                    # ENHANCED DEBUG: Analyze 8kHz PCM audio
                    samples_8k = len(pcm_8k) // 2
                    duration_8k_ms = (samples_8k / 8000) * 1000
                    max_amp_8k = max(abs(s) for s in struct.unpack(f'<{samples_8k}h', pcm_8k)) if samples_8k > 0 else 0
                    logger.info(f"[{call_id}] 📊 8kHz PCM: {samples_8k} samples, {duration_8k_ms:.1f}ms, max_amp={max_amp_8k}")
                    
                    # ENHANCED DEBUG: Check for silence
                    if max_amp_8k < 100:
                        logger.warning(f"[{call_id}] ⚠️ WARNING: 8kHz PCM appears to be silent (max_amp={max_amp_8k})")
                    
                    # ENHANCED DEBUG: Show first 10 samples
                    first_samples = struct.unpack(f'<{min(10, samples_8k)}h', pcm_8k[:min(20, len(pcm_8k))])
                    logger.info(f"[{call_id}] 📊 First 10 samples: {first_samples}")
                    
                    # COMPLETE CALL RECORDING: Add AI audio to call recorder
                    call_recorder = call_info.get('call_recorder')
                    if call_recorder:
                        call_recorder.add_ai_audio(pcm_8k)
                    ...
                    # BARGE-IN SUPPORT: Check if this turn is still current before sending
                    if call_info.get('current_turn_id', 0) != turn_id:
                        logger.info(f"[{call_id}] 🚫 TTS playback cancelled - turn {turn_id} is no longer current (current: {call_info.get('current_turn_id', 0)})")
                        return
                    
                    # CRITICAL FIX: Prevent race conditions - only allow one audio send at a time
                    if call_info.get('audio_sending_in_progress', False):
                        logger.warning(f"[{call_id}] Audio send already in progress - skipping to prevent race condition")
                        return
                    ...
                    try:
                        # BARGE-IN SUPPORT: Create cancellable TTS playback task
                        tts_task = asyncio.create_task(self._send_tts_audio_with_turn_control(call_id, pcm_8k, turn_id))
                        call_info['tts_playback_task'] = tts_task
                        
                        # Wait for TTS playback to complete or be cancelled
                        try:
                            success = await tts_task
                        except asyncio.CancelledError:
                            logger.info(f"[{call_id}] 🚫 TTS playback task was cancelled (barge-in detected)")
                            success = False
                        finally:
                            call_info['tts_playback_task'] = None
                        
                        if success:
                            logger.info(f"[{call_id}] 🔊 Sent TTS audio to Vicial: {len(pcm_8k)} bytes (real-time chunked)")
                            call_info['audio_frames_received'] += 1
                            
                            # CRITICAL FIX: Mark audio streaming as complete
                            call_info['audio_streaming_complete'] = True
                            
                            # Save TTS audio for debugging if enabled
                            if SAVE_AI_AUDIO:
                                await self._save_tts_audio_response(call_id, tts_wav_data, pcm_8k)
                            
                            # DEBUG: Inject test tone after TTS to verify AudioSocket is still active
                            await asyncio.sleep(0.5)  # Small delay after TTS
                            await self._inject_mid_call_test_tone(call_id, call_info['vicial_writer'], "beep")
                        else:
                            logger.error(f"[{call_id}] Failed to send TTS audio to Vicial")
                            # Mark as failed so cleanup knows
                            call_info['audio_streaming_failed'] = True
                    except Exception as e:
                        logger.error(f"[{call_id}] Exception during TTS audio playback: {e}")
                        call_info['audio_streaming_failed'] = True
                    finally:
                        # Always clear the in-progress flag
                        call_info['audio_sending_in_progress'] = False
                else:
                    logger.error(f"[{call_id}] Failed to downsample TTS audio from 16kHz to 8kHz")
            else:
                logger.error(f"[{call_id}] Failed to convert TTS WAV to PCM")
                        
        except Exception as e:
            logger.error(f"[{call_id}] Error sending TTS audio to Vicial: {e}")
