...
from config import DEFAULT_CONVERSATION_TURNS, AVAILABLE_VOICES, VAD_ENABLED
...
from audio_utils import debug_audio_bytes, validate_audio_format
...
class WebSocketHandler:
    ...
    async def handle_connection(self, websocket):
        ...
        try:
            ...
            self.active_connections[connection_id] = {
                ...
                "vad": None,  # VAD instance for this connection
                "barge_in_count": 0,  # Track barge-in events
                "last_barge_in_time": 0  # Timestamp of last barge-in
            }
            
            # Create VAD instance for this connection if VAD is enabled
            if VAD_ENABLED:
                vad_instance = vad_manager.create_vad_for_connection(connection_id)
                self.active_connections[connection_id]["vad"] = vad_instance
                if vad_instance:
                    self.logger.info(f"VAD instance created for connection {connection_id}")
                else:
                    self.logger.warning(f"Failed to create VAD instance for connection {connection_id}")
    ...
    async def _handle_audio_message(self, connection: Dict[str, Any], audio_bytes: bytes):
        ...
        # Validate audio format at entry point
        is_valid, format_info = validate_audio_format(audio_bytes)
        if not is_valid:
            self.logger.error(f"Invalid audio format from {client_address}: {format_info}")
            await safe_send_response(websocket, f"Error: Invalid audio format - {format_info}", client_address)
            return
        
        # Debug audio information
        debug_audio_bytes(audio_bytes, f"WebSocket Audio (connection: {connection_id})")
        ...
        if connection["waiting_for_audio"]:
            ...
            # VAD Processing: Filter audio through Voice Activity Detection
            processed_audio = audio_bytes
            if VAD_ENABLED:
                # Process audio chunk through VAD
                speech_segment = vad_manager.process_audio_chunk(connection_id, audio_bytes)
                if speech_segment:
                    processed_audio = speech_segment
                    self.logger.info(f"VAD detected speech segment: {len(speech_segment)} bytes")
                else:
                    # VAD filtered out silence/noise
                    self.logger.debug(f"VAD filtered silence for {client_address}")
            ...
            response_text = await model_manager.process_audio(processed_audio, combined_turns, connection_id)
            ...
            if tts_audio:
                # UltraVAD Interruption Detection - Check for disturbances before sending
                is_interruption, confidence = ultravad_manager.detect_interruption(processed_audio, combined_turns)
                
                if is_interruption:
                    self.logger.warning(f"🚨 UltraVAD detected interruption (confidence: {confidence:.3f}) - Stopping audio transmission to {client_address}")
            ...
            asyncio.create_task(log_transcription(processed_audio, connection_id))
    ...
    async def _process_tts_request(self, websocket, client_address: str, audio_bytes: bytes, connection: Dict[str, Any]):
        ...
        if VAD_ENABLED:
            ...
            if speech_segment:
                ...
                # VAD filtered out silence/noise
                self.logger.debug(f"VAD filtered silence for {client_address}")
                await safe_send_response(websocket, "No speech detected in audio", client_address)
                return  # Skip processing if no speech detected
        
        # STEP 1: Transcribe user input first (this is critical for proper context)
        self.logger.info(f"Transcribing user input for {client_address}")
        user_transcription = await model_manager.transcribe_audio(processed_audio, connection_id)
        transcription_manager.add_user_transcription(connection_id, user_transcription)
