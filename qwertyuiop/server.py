...
class UltravoxServer:
    ...
    async def start(self):
        ...
        try:
            # Initialize ultraVAD model
            self.logger.info("Initializing ultraVAD model...")
            if ultravad_manager.load_model():
                self.logger.info("✅ ultraVAD model loaded successfully")
            else:
                self.logger.warning("⚠️  ultraVAD model failed to load - interruption detection disabled")
    ...
    def _log_server_info(self):
        ...
        self.logger.info(f"  - UltraVAD: {'enabled' if ultravad_manager.is_model_available() else 'disabled'}")
        self.logger.info(f"  - VAD: {'enabled' if vad_manager.enabled else 'disabled'}")
    ...
    async def shutdown(self):
        ...
        # Shutdown model manager, ultraVAD manager, and VAD manager
        model_manager.shutdown()
        ultravad_manager.shutdown()
        vad_manager.shutdown()
